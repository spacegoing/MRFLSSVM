...
if __name__ == "__main__":
    ...
    with open('theta_test.pickle', 'rb') as f:
        theta = pickle.load(f)
    for i in theta:
        print(i, end=", ")
    print()
    ...
    for t in range(0, options.maxIters):
        ...
        with open('ex_test.pickle', 'rb') as f:
            u, p, c, t = pickle.load(f)
        uu = unaryWeight * instance.unary_observed - lossUnary
        pp = instance.pairwise
        cc = instance.clique_indexes
        tt = theta
        print(np.sum(uu == u) == u.shape[0] * u.shape[1] * u.shape[2])
        print(np.sum(pp == p) == p.shape[0] * p.shape[1])
        print(np.sum(cc == c) == c.shape[0] * c.shape[1])
        print(np.sum(tt == t) == t.shape[0])
        y_loss, z_loss, e_loss = \
            mrf.inf_label_latent_helper(unaryWeight * instance.unary_observed - lossUnary,
                                        instance.pairwise,
                                        instance.clique_indexes, theta, options)
        with open('loss_test.pickle', 'rb') as f:
            b_y_loss, b_z_loss, b_e_loss = pickle.load(f)
        print(np.sum(y_loss==b_y_loss)==y_loss.shape[0]*y_loss.shape[1])
        print(np.sum(z_loss==b_z_loss)==z_loss.shape[0]*z_loss.shape[1])
        print(e_loss==b_e_loss)
